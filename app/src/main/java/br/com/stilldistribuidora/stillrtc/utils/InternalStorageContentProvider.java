...
public class InternalStorageContentProvider extends ContentProvider {
    public static final Uri CONTENT_URI = Uri.parse("content://PublicarActivity/");
    ...
    public boolean onCreate() {
        try {
            File mFile = new File(getContext().getFilesDir(), UploadFile.TEMP_PHOTO_FILE_NAME);
            if(!mFile.exists()) {
                mFile.createNewFile();
    ...
    public ParcelFileDescriptor openFile(Uri uri, String mode) throws FileNotFoundException {
        File f = new File(getContext().getFilesDir(), UploadFile.TEMP_PHOTO_FILE_NAME);
        if (f.exists()) {
